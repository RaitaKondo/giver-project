...
export function DashboardPage() {
  ...
  const [followOverview, setFollowOverview] =
    useState<FollowOverviewResponse | null>(null);
  ...
  const stats = useMemo(() => {
    ...
    const recentPosts = rawPosts.filter((post) =>
      isInLastDays(post.createdAt, 30),
    ).length;
      ...
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 max-w-xl text-slate-500">
            ログイン中のアカウントに紐づく投稿を集計しています。
          </p>
        </div>
      </header>

      {isLoading ? (
        <StatePanel message="ダッシュボードを読み込み中です..." />
      ) : null}
        ...
        <>
          <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map(([label, value, diff]) => (
              <div
                className="min-h-[120px] rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
                key={label}
              >
          ...
          <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4 text-sm text-slate-600">
                ...
                <p>
                  プロフィール編集はヘッダーの「プロフィール編集」から開けます。
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <span className="text-sm font-semibold text-slate-500">
                  {followOverview?.followingCount ?? 0}人
                </span>
              </div>
              <div className="space-y-3">
                {followingUsers.length > 0 ? (
                  followingUsers.map((user) => (
                    <FollowUserCard key={`following-${user.id}`} user={user} />
                  ))
                ) : (
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <span className="text-sm font-semibold text-slate-500">
                  {followOverview?.followerCount ?? 0}人
                </span>
              </div>
              <div className="space-y-3">
                {followerUsers.length > 0 ? (
                  followerUsers.map((user) => (
                    <FollowUserCard key={`follower-${user.id}`} user={user} />
                  ))
                ) : (
...
function toDashboardUser(
  user: FollowOverviewResponse["followingUsers"][number],
): User {
  return {
    id: user.id,
